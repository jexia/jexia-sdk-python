...
def test(ctx, profile=False):
    ...
    with ctx.cd(ROOT):
        ctx.run('py.test {0}'.format(kwargs), pty=True)
...
def cover(ctx, html=False):
    ...
    with ctx.cd(ROOT):
        ctx.run('py.test --cov=jexia_sdk --cov-report=term {0}'.format(extra), pty=True)
...
def qa(ctx):
    ...
    with ctx.cd(ROOT):
        ...
        flake8_results = ctx.run('flake8 jexia_sdk tests', pty=True, warn=True)
...
@task(clean, deps, test, doc, qa, dist, default=True)
def all(ctx):
